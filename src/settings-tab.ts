...
export class MicaSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl).setName("Automatically Convert images at Start").setDesc("Scan the vault and convert eligible images once after Obsidian finishes loading. Off by default.").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoConvertImagesAtStart).onChange(async (value) => { this.plugin.settings.autoConvertImagesAtStart = value; await this.plugin.saveSettings(); }));
    ...
    new Setting(containerEl).setName("Review before conversion").setDesc("Off by default for one-click conversion. Turn on to preview the scan results before converting.").addToggle((toggle) => toggle.setValue(this.plugin.settings.reviewBeforeApply).onChange(async (value) => { this.plugin.settings.reviewBeforeApply = value; await this.plugin.saveSettings(); }));
