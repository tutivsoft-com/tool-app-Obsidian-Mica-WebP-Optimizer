...
- Billing requests contain the fixed Mica app identifier and a cryptographically random install identifier; checkout also includes the billing email entered in Settings. No vault paths, note contents, image bytes, or generated WebP bytes are sent.
...
Mica is designed to reduce accidental data loss and accidental metadata disclosure by staging verified output, persisting a journal before rewrites, and keeping originals by default. The optional billing relay is unsigned and rate-limited; it is used only to associate purchases with this install and spend purchased conversions, not to process media. It does not protect against a malicious or compromised Obsidian process, filesystem ransomware, another plugin modifying the same notes concurrently, or a user granting access to the vault to another application. Keep normal vault backups and review the original-file setting before a large run.
